...
public:
	...
	SearchTree(const SearchTree<S>&);							// Конструктор копирования
	...
	SearchTree<S>* ejectLeft();									// Извлечь поддерево слева
	SearchTree<S>* ejectRight();								// Извлечь поддерево справа
	...
	static SearchTree<S>* balancedTree(int n);					// Построить идеально сбалансированное дерево поиска по данному количеству элементов
	void findElement_insertLeft(SearchTree<S>*, S, S);			// Найти элемент и добавить к нему слева узел
	void findElement_insertRight(SearchTree<S>*, S, S);			// Найти элемент и добавить к нему справа узел
	void add(S dt);												// Добавить элемент в дерево
	std::vector<S> copy_to_vector();							// Копировать дерево в вектор
	SearchTree<S>* ideal_to_search( std::vector<S>, int, int);		// Преобразовать идельно сбалансированное дерево в дерево поиска
};
...
template<class S>
SearchTree<S>* SearchTree<S>::ejectLeft() // Извлечение
{
	if (this->left != nullptr)
	{
...
template<class S>
void SearchTree<S>::deleteLeft() // Удаление
{

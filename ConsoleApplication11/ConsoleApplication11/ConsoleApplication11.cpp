...
void main()
{
    ...
    else 
    {
        for (int i = 0; i < kol; i++) // цикл для случайного создания студента 
        {
            ...
            fout_rand << "Адрес проживания: " << "Г.Пермь, " << address << " " << getRand(100) << ", кв." << getRand(200) << endl;
    ...
    ifstream fin; 
    fin.open("Test.txt", fstream::in); // открытие для чтения переменной fin файла "Test.txt"
    fout_final.open("Test_fin.txt"); // открытие для записи переменной fout_final файла "Test.txt"
	...
	else
	{
		...
		while (!fin.eof()) // цикл отрабатывает, пока не найдет конец файла
		{
			...
			if (str_stud == "Студент") // если новое слово - "Студент", то
			{
				...
				{
                    fout_final << str_stud << " " << str1 << endl << str2 << endl; // вывод на экран

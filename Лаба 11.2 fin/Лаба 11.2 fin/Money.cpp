...
Money& Money::operator-=(const Money& money)
{
	long long money_this = (long long)m_rubles * 100 + m_kopeck;
	long long money_other = (long long)money.m_rubles * 100 + money.m_kopeck;

	m_rubles = (money_this - money_other) / 100;
	m_kopeck = (money_this - money_other) % 100;
...
{
	return (out << money.m_rubles << ":" << money.m_kopeck);
}
